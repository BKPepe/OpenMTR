...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
  ...
  setupUi();
  connect(
      qApp, &QApplication::focusChanged, this, [this](QWidget *, QWidget *now) {
        bool isInteractive =
            now &&
            (now->inherits("QPushButton") || now->inherits("QCheckBox") ||
...
void MainWindow::setupUi() {
  ...
  m_targetEdit->setStyleSheet("border: none; border-radius: 0; background: "
                              "transparent; padding: 0px 6px 0px 9px;");
  m_targetEdit->setContextMenuPolicy(Qt::NoContextMenu);
  m_targetEdit->setGeometry(
      0, 0, 295, 32); // right edge trimmed at runtime for the clear button
  connect(m_targetEdit, &QLineEdit::returnPressed, this,
...
#else
  auto *clearBtn = new QPushButton(QString(QChar(0x2715)), targetWrap);
#endif
  ...
  auto updateClear = [this, clearBtn, targetWrap]() {
    const bool show =
        m_targetEdit->hasFocus() && !m_targetEdit->text().isEmpty();
  ...
  m_pingSizeBox->setStyleSheet(
      "QSpinBox { border: none; border-radius: 0; background: transparent; "
      "padding: 0px 6px 0px 9px; } QSpinBox QLineEdit { border: none; "
...
void MainWindow::updateInputWrapStyle(QFrame *wrap, InputAccentBar *accent,
  ...
  if (active) {
    wrap->setStyleSheet(QString("QFrame { border: 1px solid %1; border-radius: "
                                "4px; background-color: %2; }")
                            .arg(sideColor, activeBg));

    if (accent) {
      const int thickness = 2;
      accent->setGeometry(0, wrap->height() - thickness, wrap->width(),
                          thickness);
...
void MainWindow::setTracingInputsEnabled(bool enabled) {
  ...
  for (auto *w :
       std::initializer_list<QWidget *>{m_targetEdit, m_pingSizeBox}) {
  ...
  if (event->type() == QEvent::FocusIn || event->type() == QEvent::FocusOut) {
    ...
    if (w) {
      ...
      if (auto *frame = qobject_cast<QFrame *>(w->parent());
        ...
        updateInputWrapStyle(frame,
                             frame->findChild<InputAccentBar *>("inputAccent"),
                             inputActive, focused);
      }
    }
  }
...
void MainWindow::mousePressEvent(QMouseEvent *event) {
  ...
  bool isInteractive =
      clicked &&
      (clicked->inherits("QPushButton") || clicked->inherits("QCheckBox") ||
